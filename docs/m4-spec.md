...
| **T4.21 `DisasterSystem` + `DisasterRow` (v25)** (LIVING revision, CR-015) | every snapshot (layout + RngStreams); food production of struck settlements → stores → deficits → every deficit reader; chronicle; `Cr003Quarantine` guards | ALL FOUR GOLDENS + `ci.yml:136` (layout at T4.21-1 with λ = 0; behaviour at T4.21-4); `IntegratedPinAttribution` ladder extended; `SnapshotDiff` layout; `Assert.Equal(24, Version)` ×4 + a JSON literal; v24 UI saves unloadable |
| **T4.21 `FoodState` / `d_eff`** | Demographics starvation and fertility flows; chronicle famine events | `PopulationExactnessTests` d > 0 rigs (replica takes `dEff`); `Famine_MortalitySpike` re-anchored (G2); `ClassSystemTests` famine-drain timing |
| **T4.21 `FoodHeadroom` (`N_lim`) + headroom cap** | growth of near-limit fed settlements (down-only); vacancy of destinations; `FedGrowth_DtInvariant` (identical); density corridor (down-only) | goldens only where a canonical settlement has small `H_0` or a refugee inflow exceeds `cap_j` — ladder step "P3 alone" |
| **T4.21 bounded flight + ω + shares (nominal `d`)** | `MigrationFlows`; `UnplacedDeparture` (colonization party sizes ≈ −⅓ at the rigs' deficits); `CollapseStabilityTests` trajectories; `Overdraw_ScalesProportionally` (re-rigged to shares); ordered-golden worlds | goldens; `ObservedWorlds`/`WorldReconciliationTests` measured-turn comments; `Founding5` party/provisions comments; every `ColonizationTests` rig re-measured |
| **T4.21 basin caps (gap channel) + vacancy bound (total inflow)** | multi-source/destination gap flows; refugee inflow into small or land-bound destinations after a collapse; `queue.md:65` ping-pong; `BifurcationConfig_NoTwoTurnOscillation` (must NOT regress) | goldens (behaviour); new many-to-one / one-to-many pins; `M_Basin_FanIn_Flight_BoundedByVacancy` |
| **T4.21 observability statics** | telemetry field set (additive under `telemetry/v2`); `MigrationExplanation` text; UI positional ctors | no golden (`TelemetryTests :19` pins no hash change) |
...
| **T4.21 — famine semantics, bounded migration, shock integration** (post-certification packet, the T4.17–T4.20 class; spec `docs/t4.21-architecture.md`, sub-packets T4.21-0..6) | derived four-state `FoodState` (FAMINE iff `d > 0` AND a famine-class disaster or productive abandonment); `DisasterSystem` + `DisasterRow` (schema v25) as a production shock; effective deficit `d_eff` into starvation and fertility (G3(b)); exact flight hazard on the best exit, basin caps, the vacancy bound; the feedable food-influx limit and the headroom growth cap (§28). No constant moved, no band moved, weather untouched (G1 escalated) | **CR-015 ruled** (`docs/adr/cr-015-famine-is-exceptional.md`, mandate 2026-09-17); ADR-024/025/026; T4.20 | **YES** (mutants M-FS-*, M-MIG-CAP/FANIN/FANOUT/SUMNOTMAX/VACANCY, M-DEM-CAP/ADAPT/SUMFOOD/BANKCAPPED; independent worktree per verifier) |
