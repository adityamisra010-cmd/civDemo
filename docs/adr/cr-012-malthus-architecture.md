# CR-012 — MALTHUSIAN FEEDBACK ARCHITECTURE: KEEP / SIMPLIFY / REPLACE

**RENUMBERED 004 -> 012 ON INTEGRATION.** This document was authored as CR-004 on
`t4.10-migrated-work`, in parallel with an unrelated CR-004 authored on the food-anomaly branch
(`docs/adr/cr-004-granary-turn-length-mismatch.md`, withdrawn by its own evidence). Both reached
`main` by different routes and the number collided. The granary CR keeps 004 because it merged
first; this one takes the next free number. 008-010 are NOT free: `docs/queue.md` already refers to
CR-008, CR-009 and CR-010 by number for open questions that have no file yet, and reusing one would
