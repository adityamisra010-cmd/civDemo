...
| **K-21 — CORRECTION (recorded, not rewritten): the latch was misdescribed in two prior records as *"records that a predicate has fired"*.** *"**The shipped latch records CURRENT satisfaction under hysteresis, not history** … Any design relying on capability outliving its preconditions must say so explicitly and justify it."* | `docs/milestone-architecture-governance.md:242-251` (§6.2b); correction notice `docs/capability-architecture-decision.md:20-23`; the misdescriptions are `docs/m5-roadmap-dependency-audit.md:220-221` and `docs/capability-architecture-decision.md:128-131` | RATIFIED (as a correction to secondary records; the shipped semantics are K-20) | Kills the casual argument that "a government change transforms rather than deletes acquired capability, because a latch remembers". It does not remember. |
