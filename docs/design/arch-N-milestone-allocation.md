...
| **G8(c) / CR-016 option 3 — the per-year food-balance sub-step** | *"option 3 … remains queued as **M5-scale work**"* | `docs/adr/cr-016-armed-disaster-fallout.md:305-310`; the queue entry is `docs/queue.md:1403` (G8(c)) — **MEASURED**. The option is not adopted: CR-016 itself is **OPEN** |
