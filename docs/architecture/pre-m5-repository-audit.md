...
| 3 | band edges are turn boundaries; the campaign is exactly 1,630 turns | `EraTableLoader.cs:106-109`; ADR-002 `:26-30`; `EraTableTests.cs:30,52-54` | any turn whose length ≠ band dt: a shortened turn lands mid-band; the next lookup still works (half-open bands) but the edge-landing guarantee and the 1,630 pin die | ADR-002 consequence |
