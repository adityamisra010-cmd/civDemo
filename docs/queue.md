...
- **T4.21-2 — the turn-2 vacancy refusal (measured).** The founding turn harvests zero (the T4.18 warm-up artefact), so on turn 2 every settlement reads `N_lim = 0 ⇒ V = 0` and the vacancy bound refuses every gap flow world-wide for that one turn (founded and driven seed 42: migration's first turn moved 2 → 3). The spec's null arm covers the ABSENT row (turn 1), not the zero-harvest row; a one-line rule ("no harvest row yet ⇒ V = +∞") or the T4.18 warm-up fix would close it. Reported, not acted on (a mechanism boundary, not a tuning). **CONFIRMED ON THE MERGED T4.21-2 ∥ T4.21-3 TREE** by the merge-finish lane, measured: `MigrantsMoved` is 0 on turn 2 and 252 on turn 3 in BOTH the founded and the driven seed-42 world, and T4.21-3's prose/asserts that assumed a turn-2 move (WorldReconciliationTests' driven "all five flows", the ObservedWorlds doc) were re-written to the measured turns. The downstream readings moved with it: founded first trade 41 → 28, driven first dwelling decay 48 → NEVER in 300 turns. Still a mechanism boundary; still not acted on here. **T4.21-4 (RULE 2) APPLIED, STILL OPEN, 2026-09-18:** `FoodHeadroom.Limit`'s null arm now includes catchment ROW ABSENCE (ADR-026 §2.1a), keyed on row absence only — never on `production == 0`, which is the abandoned settlement's genuine zero — with `H_NullArm_NoCatchmentRow_IsPositiveInfinity_ButAbandonedWithARowIsZero` pinning both arms. MEASURED: it does NOT close this line. The change is a bit-exact no-op on the canonical founded world (300-turn hash `db7c7a09…` before and after, the ci.yml FOUNDED_GOLDEN reproduced via the ci step; gross/decade 0.001021 both ways; first migration turn 3 both ways) and byte-identical over every turn of the driven world. Catchment is pipeline entry 1 and writes into NEXT, so the turn-1 world — PREV on turn 2 — already carries a row for every founded settlement; the turn-2 refusal comes from that row being PRESENT alongside the turn-1 zero staple harvest, which the rule forbids keying on. Closing it needs a key separating "the row did not exist when the production ran" from "the row existed since founding" (not computable from prev alone without a new field or a warm-up marker) or the structural T4.18 warm-up fix. See ADR-025 §2.4a. **DEMOGRAPHIC HALF (T4.21-4 fix lane, measured):** the same structural zero binds the DEMOGRAPHIC growth cap on turn 2 as well — with `S = 0` world-wide, `N_lim = 0 ⇒ H_0 = 0` and births are held to replacement in EVERY settlement for that one turn. Measured on founded seed 42 under the director's 15-order `--labor` corpus: turn 1 harvest 0 in 12/12 settlements, world opening 5143, births 2157, deaths 2055 → 5245; turn 2 births 2070, deaths 2167 → 5148, with births ≤ deaths in 12/12 settlements against 4/12 on turn 1 and 6/12 on turn 3 — population falling on a turn with no deficit and no suppression. The downstream pins that moved for it are recorded in `docs/t4.21-3-mutants.md` (artisan latch 39 → 43 at `:132`/`:189`, housing draw 500 → 484 at `:140`, the turn-3 founded deficit and grain-demanded pins). So a ruling on this line is a ruling on migration AND demographics, not migration alone. **WHY NO KEY CAN CLOSE IT:** RULE 2's acceptance criterion (first migration turn returns to 2; this line closed) is NOT MET and is not attainable under RULE 2's own prohibition on keying the arm on `production == 0` — the rule's premise that no catchment row exists on turn 1 is refuted by `pipeline.json` entry 1 + `CatchmentSystem.cs:121-141`/`:178`, so there is no row absence on turn 2 to key on. Recorded as a DEVIATION needing director sign-off in ADR-025 §2.4a.
- **T4.21-2 — basin caps bite constantly in the fed world (measured, for the director).** On founded seed 42 the destination/source basin caps (ADR-025 §2.3) bite in 8–12 of 12 settlements per early turn and cut the pair-capped gap inflow ~3× (14 863 pair-capped vs 4 941 moved over 300 turns) — spec §3.5b expected them to bite "on the multi-source refill after a collapse"; with 11 destinations per source every basin is multi-member. Downstream: first merchant latch moved (t124 now, receded by 650), first starvation 55 → 57, first dwelling decay (driven) 48 → 64. The bound is the specified physics (fan-out IN SCOPE); whether its fed-world magnitude is wanted is a director question, not a threshold to move. **CLOSED BY T4.21-4 (RULE 1, measured), 2026-09-18:** the pre-registered test was applied on the canonical founded world (seed 42, no orders) and the source basin cap **STAYS as shipped** — gross migration per decade over turns 2..300 is 0.001021 on the merged tree against 0.001509 pre-packet (`1735d41`), i.e. 0.677x, which clears the 0.4x threshold (0.000604) and sits inside the corridor band [0.001, 0.01]. No mechanism changed and no test or mutant was removed. The fed-world magnitude is the specified physics, measured and ACCEPTED (ADR-025 §2.3a, `docs/t4.21-2-record.md` §9.1). Recorded alongside it, from the diagnostic arms: neither basin cap is what moved pre-packet → merged (source off 0.000922, destination off 0.000939 — both DOWN), the source cap's evidence is per-turn rather than 300-turn (driven seed 42 max inflow fraction 0.1754 → 0.1987 with it off), and the ~3x headline above is mostly the DESTINATION stage (source binds 13.8 % of settlement-turns, destination 37.4 %). Where the residual pre-packet→merged cut actually sits is ESCALATED to the director, not decided here. **RULE 1 EVIDENCE COMPLETED (T4.21-4 fix lane, measured):** RULE 1 named four worlds and the original table carried seed 42 only. All four arms re-measured on the lane's own worktrees (arm (a) `1735d41`, arm (b) `3671ed0`; seed 42 reproduces the recorded 0.001509 / 0.001021 and both world hashes): seeds 42/1/2/3 give (a) 0.001509 / 0.000968 / 0.001406 / 0.001736 → (b) 0.001021 / 0.000795 / 0.000799 / 0.001131, ratios 0.677 / 0.821 / 0.568 / 0.652. The RATIO half holds on all four; the BAND half fails on seeds 1 and 2 — seed 1 was ALREADY below the 0.001 floor pre-packet (0.000968), but **seed 2 CROSSES OUT of the band because of the packet** (0.001406 → 0.000799). The STAYS ruling rests on reading 'arm (b)'s canonical gross migration' as the seed-42 canonical world, with seeds 1–3 supporting the ratio half only, and `corridors.json` ruling migration a record rather than an acceptance gate. Seed 2's band exit is the largest of the four cuts and belongs with the already-escalated question of where the pre-packet → merged residual sits (ADR-025 §2.3a).
- **T4.21-4 — ARMING THE DISASTER BREAKS THREE FROZEN GATES. ESCALATED: `docs/adr/cr-016-armed-disaster-fallout.md` (OPEN). DISARMED BY T4.21-7, 2026-09-18 — THE MECHANISM SHIPS COMPLETE AND TESTED BUT INERT AND THE RATE IS THE DIRECTOR'S; the six reds below are RESOLVED on the shipped tree and the goldens returned to their pre-arming constants byte for byte. The CR stays OPEN: what is settled is the TREE, not the question.** `sim.json disaster.hazardPerYear` 0.0 → 0.01 (the derived, ruled value). The onset process is correct and measured — 6,211 onsets over 66,000 canonical settlement-turns = 0.941061 per settlement-century, inside the binomial 99 % band [0.922204, 0.981047] at z = −0.925 — and so is the ladder (0 of 1,540 FAMINE settlement-turns with reason None; 0 starvation on every turn with nothing above STRESS; λ = 0 gives ZERO famine while still giving 352 STRESS). What the ruling did not anticipate is the MAGNITUDE against the demographic kernel and the G8/F4 dt artefact becoming load-bearing. MEASURED: the canonical world loses ~74 % of its turn-300 population (median armed÷unarmed 0.26 over 20 seeds); `canonical.fedGrowthPerYear` 0.000407442 against the band [0.0005, 0.001]; the CR-001 PERMANENT dt-continuity detonator breaks across the turn-250 dt 10 → 5 gate by 2.5259 per 1000 yr (+0.4352 before, −1.8220 after); the dev world goes 93,910 → 38 (seed 42) and 123,600 → 62 (seed 7) over 1000 turns. Arithmetic: at 9.5 % of settlement-decades taking an onset and a FAMINE turn's mortality 1 − e^{−0.12·d·10} (38 % at d = 0.4) against a fed clock of 0.76 %/decade, the expected drift is −2.9 %/decade — net negative by construction. The hazard and the mortality kernel were derived independently and their product was never checked. NO BAND, CONSTANT OR DERIVATION WAS MOVED; six tests are left RED and named in CR-016 §5.
- **T4.21-4 — ADR-018 §11's premise no longer holds (measured, NOT actioned).** The skip on `MagnitudeCorridor_FedPhaseDrift_WithTeeth` rested on gap-driven migration responding ×3.10 to a 10× rate lever while GROSS responded ×1.07. Re-measured on the armed tree (dev preset, full window): gap-driven 0.752 / 1.011 / 1.902 %/decade at 0.1× / 1× / 10× (×0.74 / ×1.88) and GROSS 0.692 / 0.972 / 1.852 (×0.71 / ×1.91) — the two observables now respond almost identically. T4.21-2's bounded flight and basin caps changed what GROSS is made of. The teeth were re-aimed onto the gap channel because that is what CR-015 N9 ruled; whether GROSS is still "the wrong observable" is now an open question, printed by the test.
- **T4.21-4 — the CR-003 Malthus teeth need a cause-attributed re-aim (NOT taken).** `AssertMalthusKnownDeviation`'s three teeth (`starvedTotal == 0`, `crashes == 0`, `peak == final`) all assert the pre-Malthusian premise and all three now fail on dev seeds 42 and 7 — by a DISASTER, not by a land ceiling. CR-015's dispositions authorise re-READING the `starvedTotal == 0` tooth; re-aiming all three onto "the starvation and the crash are famine-attributable" is a change to the CR-003 quarantine's substance and is the director's. Left RED and named (CR-016 §5).
- **T4.21-5 — `recentStress` is NOT SHIPPED.** Spec §3.11's last row asks the forensic reader to DERIVE "turns since the last SEVERE/FAMINE settlement-turn" over the telemetry series. The T4.21-5 packet brief's own SHIP list does not carry it, and CLAUDE.md's "implement exactly the packet you are given" decides the conflict in favour of the brief. Everything it needs now exists in the record (`foodState.state` per settlement-turn, telemetry/v3), so it is a pure reader derivation whenever the director wants it — plus the question its placement raises: which existing answer gains the column, since `SettlementHistory`'s format is pinned.
- **T4.21-5 — `migrationPlan.inflowAeByChannel` is a GAP, stated not numbered.** `MigrationPlan.DesiredInflowAe` is ONE adult-equivalent figure over the gap AND flight channels together, because that is what the vacancy bound compares against; the planner forms no per-channel split, so the observer records none (the per-channel HEAD counts ARE recorded). Splitting it would be an observer-side re-derivation of a quantity the simulation does not compute — forbidden by `docs/observability-architecture.md` §0. If the split is wanted, the planner is where it must be formed.
- **T4.21-5 — `MigrationExplanation.ViabilityProducts` and `.Damping` are now RECOMPUTABLE but still say "not recorded".** Both are per-destination / per-pair values the public `MigrationSystem.Plan` carries (`plan.Viability`, `plan.Damping`); T4.21-5 exposed the per-settlement bounds beside them and reworded only `.GapScale`, leaving these two strings untouched to keep the packet's blast radius at what was asked for. They remain literally true (this explanation does not record them) but a reader could reasonably expect the viability product beside its three inputs now that the planner is public.
- **T4.21-6 — the turn-2 REFUGEES REFUSED burst is the founding-harvest warm-up artefact (narration corrected, cause still open).** On the armed canonical session (seed 42, 60 turns, founded, measured on this tree) 11 of the 43 `REFUGEES REFUSED` lines in the whole run fall on turn 2, one for 11 of 12 settlements, every one with `vacancyCap 0` and `vacancy 0`. Nothing was refused: there was no capacity to refuse from. Same cause as the two `T4.21-2` lines above — `ProductionSystem.Farm` reads `prev.CatchmentSummaries`, absent on turn 1, so on turn 2 every settlement reads a zero food influx (`docs/queue.md` above; ADR-025 §2.4a; ADR-026). `SessionInspector` now prints the neutral form on that shape ("the destination had no vacancy at all this turn") instead of "the destination fed what it could and turned the rest away", and `InspectionTests` pins it in both directions. The EVENT is not suppressed — only its narration of a decision the record does not contain. The burst itself disappears when the founding-harvest warm-up fix closes those lines; it is not closed here.

- **T4.21-7 — THE DISARMED TREE'S OWN OPEN ITEMS (what the CR-016 ruling would change back).** The famine-class disaster ships INERT (`sim.json disaster.hazardPerYear` 0.0; band, duration and derivation untouched), so three dispositions currently read as quarantines rather than coverage, each with BOTH measurements recorded at its site and CR-016 named as what decides it: (1) `PopulationTests.Reconciliation_FromLedgerAlone`'s `starved > 0` guard — measured 0 starvation over the 900-turn dev rig at λ = 0 against 5,983 / 6,414 armed, so the ledger reconciliation covers three Population reasons and not four; (2) `ChronicleTests.Annals_TwinIdentical`'s famine line — 20 chronicle events, 0 of them famine, so the annals twin compares a chronicle without that event class; (3) `MigrationTests.MagnitudeCorridor_FedPhaseDrift_WithTeeth`'s UPWARD rate-lever tooth — ×1.0082 at λ = 0 against ×1.88 armed, BELOW ADR-018 §11's own recorded dead signature of ×1.07, so the gap-closing cap binds in every pair again and ADR-018 §11's premise returns; its corridor assertion and both downward teeth stay live and were NOT skipped. Also standing: `WorldReconciliationTests`' founded-starvation and driven-dwelling-decay coverage is back to ABSENCE pins (zero on all 300 turns, measured), because at λ = 0 no cause reaches those accounts in those worlds. None of this is a defect; all of it is the cost of shipping the rate unset, stated rather than hidden. Every one reverses with the same single data edit.
- **T4.21-7 — ADR-018 §11's premise, re-measured at the SHIPPED λ (open question, NOT actioned).** T4.21-4 recorded that on the armed tree gap-driven and gross responded almost identically (×0.74/×1.88 against ×0.71/×1.91), refuting §11's ×3.10-vs-×1.07 discrimination. Measured at λ = 0 by T4.21-7, same rig: gap-driven 0.552 / 0.64876 / 0.65408 %/decade and gross 0.520 / 0.602 / 0.617 at 0.1× / 1× / 10× — again almost identical, and now DEAD upward on BOTH observables. So the question §11 raised (is GROSS the wrong observable, or is the LEVER unreachable?) is answered differently at the two rates and is open at both. Whoever re-derives it needs both readings.
- T4.21-8 (from finding 2, a better way, NOT taken — CLAUDE.md "a better way exists is not a conflict"): SimConfigTests anchors the disaster-arming substitutions on the SHIPPED literal `"hazardPerYear": 0.0,`, so the director ruling a rate re-aims six tests (priced in cr-016 §D.3). A value-agnostic anchor (regex on the key, substituting whatever value is there) would make all six survive any shipped rate and shrink the ruling's bookkeeping to the tripwire alone. Not taken here because `DisasterHazard_Armed_Loads` deliberately pins that arming is a substitution from the SHIPPED literal, and changing what it pins is outside T4.21-8's finding.
- T4.21-8 (finding 1, ambiguity RECORDED per gov-4 §6, frozen documents NOT rewritten): four T4.21-4-era records — `docs/t4.21-4-record.md` §5 and its red table, this file's T4.21-4 entry, and `docs/adr/adr-024-food-state-effective-deficit-disaster-shock.md` — pair the discontinuity **2.5259** with the components **+0.4352 / −1.8220**. Those are TWO DIFFERENT RIGS: the components are `CalibrationBatteryTests.Canonical_EraBoundaryContinuity_PermanentBatteryMember` (seed **1**, windows 1600–2500 / 2500–3400), whose own discontinuity is **2.2572**; the 2.5259 is `DemographyRetuneTests.EraBoundaryContinuity_NeolithicToBronze_PermanentDetonator` (turns 1..450), whose components are **+0.7616 / −1.7643**. Both re-measured by T4.21-8 at λ = 0.01; both ~23–25× the 0.1 bar, so NO conclusion changes. The single correction is `docs/adr/cr-016-armed-disaster-fallout.md` §D.6 — read those four sites through it. The frozen records are left alone deliberately.
