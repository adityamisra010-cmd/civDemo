...
| 2 | per-settlement spoilage vs granary overflow | `ConsumptionSystem.BoundStore` (:296-323) | both computed per settlement, recorded only as world ledger totals | SETTLEMENT/Food: "store losses" residual with its identity string; Σ equals the ledger, asserted | two `long`s on the grain `GoodStockRow` or a `StoreLossRow` |
