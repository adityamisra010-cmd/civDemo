...
public class PipelineLoaderTests
{
    ...
    public void CanonicalPipelineFile_LoadsInConfiguredOrder()
    {
        ...
        Assert.Equal(17, pipeline.Length);   // T4.5 `appropriation`; T4.4 `colonization`; M4-D `construction`; T4.13 `revolt`; T4.21-1 `disaster`
        ...
        // T4.21-1 (CR-015 §3.3): the famine-class disaster is drawn immediately
        // AFTER weather and BEFORE production, written for NEXT turn's PREV read
        // exactly as weather is. Production reads PREV either way, so the slot is
        // legibility ("stochastic drivers first") plus the turn-1 RngStreams
        // creation order, which is hashed — pinned so a reorder is deliberate.
        Assert.Equal("disaster", pipeline[2].Name);
        Assert.Equal("production", pipeline[3].Name);
        ...
        Assert.Equal("appropriation", pipeline[4].Name);
        Assert.Equal("consumption", pipeline[5].Name);
        ...
        Assert.Equal("construction", pipeline[9].Name);
        Assert.Equal("classmobility", pipeline[10].Name);  // T2.2, spec §3 pipeline order
        Assert.Equal("migration", pipeline[11].Name);     // T2.5, spec §3 pipeline order
        ...
        Assert.Equal("colonization", pipeline[12].Name);
        ...
        Assert.Equal("revolt", pipeline[13].Name);
    ...
    public void UnknownSystem_FailsNamingItAndListingKnown()
    {
        ...
        Assert.Contains(
            "known systems: catchment, harvestweather, disaster, production, appropriation, consumption, price, trade, housing, construction, classmobility, migration, colonization, revolt, demographics, needsgrievance, pathbuild, weather, growth, toytrade",
            e.Message);
    }
